// src/tabs/WeightTab.jsx
import { doc, setDoc } from "firebase/firestore";
import { db } from "../firebase";
import { generateWeightProjection, deriveMilestones } from "../constants/weightPlan";

export default function WeightTab({ ctx }) {
  const {
    userId, weightLog, setWeightLog, weightPlanConfig, editingPlan, setEditingPlan,
    editCfg, setEditCfg, savePlanConfig, S,
  } = ctx;
  ...
  return (
    <div style={{ ...S.main, display:"flex", gap:"14px", alignItems:"flex-start", padding:"16px" }}>
      ...
      <div style={{ flex:"0 0 55%", minWidth:0 }}>
        <div style={{ fontSize:"15px", fontWeight:"bold", color:"#1F4E79", marginBottom:"10px" }}>Weekly Weight Log</div>
          ...
          <div style={{ padding:"10px 12px", fontSize:"11px" }}>
            ...
            {editingPlan ? (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"8px", marginBottom:"10px" }}>
                ...
                <div><span style={lbl}>VO2 Max</span>{num("vo2max",54)}</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3px 12px", marginBottom:"10px" }}>
                {[["Age",`${cfg.age} yr`],["Sex",cfg.sex==="m"?"Male":"Female"],["Height",`${cfg.heightCm} cm`],
                  ...
                  ["VO2 Max",`${cfg.vo2max} - ${cfg.vo2max<35?"Fair":cfg.vo2max<45?"Good":"Excellent"}`]
                ].map(([k,v])=>(
            ...
            {/* Daily Exercise Plan */}
            <SLabel>Daily Exercise Plan</SLabel>
            {editingPlan ? (
            ...
            {/* Milestones */}
            <SLabel>Milestone Roadmap</SLabel>
            <div style={{ marginBottom:"10px" }}>
              {milestones.map((m,i) => (
            ...
            {editingPlan ? (
              ...
              <div>
                {[["Lever 1",`Extend PM to ${cfg.lever1PmMin} min`,"Easiest - no intensity change"],
                  ["Lever 2",`Drop to ${cfg.lever2CaloriesKcal.toLocaleString()} kcal/day`,"Recalculate at new bodyweight first"],
                  ["Lever 3","2-week diet break at maintenance","Resets leptin and adaptive thermogenesis"],
                ].map(([l,action,note])=>(
                  <div key={l} style={{ padding:"4px 0", borderBottom:"1px solid #F0F4F8" }}>
                    <div style={{ display:"flex", gap:"6px" }}>
                      <span style={{ fontWeight:"bold", color:"#1F4E79", minWidth:"46px" }}>{l}</span>
...
function SLabel({ children }) {
  return (
    <div style={{ fontSize:"10px", fontWeight:"bold", color:"#2E75B6", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:"6px" }}>
      {children}
    </div>
  );
}
