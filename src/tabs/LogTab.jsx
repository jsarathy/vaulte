          ...
          <div key={meal.id} style={{ background:"#fff",border:`0.5px solid ${C.border}`,borderRadius:"8px",marginBottom:"8px",overflow:"hidden" }}>
            ...
            <div
              ...
              <div style={{ display:"flex",alignItems:"center",gap:"7px",minWidth:0 }}>
                {/* Collapse chevron */}
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke={C.hint} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
                ...
                <span style={{ fontSize:"12px",fontWeight:"500",color:isExercise?C.blueText:C.text,whiteSpace:"nowrap" }}>{meal.name}</span>
              </div>

              {/* Right: subtotals inline */}
              {hasItems && (
                <div style={{ display:"flex",alignItems:"center",gap:"10px",flexShrink:0,marginLeft:"12px" }}>
                  {/* kcal prominent */}
                  <span style={{ fontFamily:FONT.mono,fontSize:"12px",fontWeight:"500",color:isExercise?C.blueText:C.text }}>
                    {fmt(mKcal)} kcal
                  </span>
                  {/* macro pills — hide on narrow */}
                  <div style={{ display:"flex",gap:"4px" }}>
                    {[["P",mProt,"blue"],["F",mFat,"amber"],["C",mNetC,"amber"]].map(([k,v,c])=>(
                      <span key={k} style={{ fontSize:"9px",fontFamily:FONT.mono,padding:"1px 5px",borderRadius:"20px",
                        background:c==="blue"?C.blueBg:C.amberBg,
                        color:c==="blue"?C.blueText:C.amberText,whiteSpace:"nowrap" }}>
                        {k} {fmt(v)}g
                      </span>
                    ))}
                  </div>
